...
$sql = "SELECT pme.staffname, pme.staffno, pme.training_title, 
...
if (!empty($subordinates)) {
    ...
    $message .= "Kindly return the completed evaluation form to the <strong>Learning & Development (L&D)</strong> by the deadline.";
    ...
    try {
        ...
        $mail->isHTML(true);
        $mail->Subject = 'Performance Monitoring Evaluation Notification - Admin Reminder';
        $mail->Body    = $message;

        $mail->send();
        echo "Email sent successfully to specified recipients.<br>";
    } catch (Exception $e) {
        echo "Email failed to send. Error: {$mail->ErrorInfo} <br>";
    }
}
