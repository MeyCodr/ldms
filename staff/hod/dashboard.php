...
<html lang="en">
					...
					<div class="panel panel-default">
						<div class="panel-heading">
							...
							<?php
								...
								// 		)
								// 		AND (YEAR(p.from_date) > 2024 OR (YEAR(p.from_date) = 2024 AND MONTH(p.from_date) >= 11))
								// 		ORDER BY 
								// 			CASE 
													...
													?>
														<tr>
															...
															<td>
																...
																?>
																	<a href="pme/edit_pme.php?id=<?= $row['id'] ?>" 
			...
			<div class="row">
				<div class="col-md-12">
					<div class="panel panel-default">
                        ...
                        <div class="panel-body" align="center">
							<div class="row">
								<canvas id="operationChart" style="width:100%;max-width:1500px"></canvas>
							</div>
						</div>
					</div>
				</div>
    ...
    <script>
        ...
        function destroyTrainerDataTable() {
            if ($.fn.DataTable.isDataTable('#trainerdata')) {
		...
		function makechartpublicojt(startdate,enddate) {
			$.ajax({
				...
				success:function(data)
				{
                    var ctxPublicojt = getCanvasContext(canvasPublicojt);
                    if (!ctxPublicojt) {
                        return;
                    }
                    var status = [];
		...
		function maketop10chart(startdate,enddate) {
			$.ajax({
				...
				success:function(data)
				{
                    var ctxTop10 = getCanvasContext(canvasTop10);
                    if (!ctxTop10) {
                        return;
                    }
                    var category = [];
		...
		function makebusinesschart(startdate,enddate) {
			$.ajax({
				...
				success:function(data)
				{
                    var ctxBusiness = getCanvasContext(canvasBusiness);
                    if (!ctxBusiness) {
                        return;
                    }
                    var category = [];
		...
		function makedhmsbchart(startdate,enddate) {
			$.ajax({
				...
				success:function(data)
				{
                    var ctxDhmsb = getCanvasContext(canvasDhmsb);
                    if (!ctxDhmsb) {
                        return;
                    }
                    var category = [];
		...
		function makefinancechart(startdate,enddate) {
			$.ajax({
				...
				success:function(data)
				{
                    var ctxFinance = getCanvasContext(canvasFinance);
                    if (!ctxFinance) {
                        return;
                    }
                    var category = [];
		...
		function makehumanchart(startdate,enddate) {
			$.ajax({
				...
				success:function(data)
				{
                    var ctxHuman = getCanvasContext(canvasHuman);
                    if (!ctxHuman) {
                        return;
                    }
                    var category = [];
		...
		function makeoperationchart(startdate,enddate) {
			$.ajax({
				...
				success:function(data)
				{
                    var ctxOperation = getCanvasContext(canvasOperation);
                    if (!ctxOperation) {
                        return;
                    }
                    var category = [];
		...
		function makequalitychart(startdate,enddate) {
			$.ajax({
				...
				success:function(data)
				{
                    var ctxQuality = getCanvasContext(canvasQuality);
                    if (!ctxQuality) {
                        return;
                    }
                    var category = [];
		...
		function makerndchart(startdate,enddate) {
			$.ajax({
				...
				success:function(data)
				{
                    var ctxRnd = getCanvasContext(canvasRnd);
                    if (!ctxRnd) {
                        return;
                    }
                    var category = [];
		...
		$(document).ready(function () {
			$('#trainerTable').DataTable({
				...
				"lengthMenu": [10, 25, 50], // Allow changing number of entries shown
			});
