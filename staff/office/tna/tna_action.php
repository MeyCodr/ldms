...
if (isset($_POST['btn_action'])) {
    if ($_POST['btn_action'] == 'addtna') {
        ...
        $findDept = "SELECT department FROM user WHERE id = '$userid'";
        $resultDept = mysqli_query($conn, $findDept);
        ...
        for ($i = 1; $i <= $esgaware; $i++) {
            ...
            if ($taskes != '' && $traininges != '') {
                $sql = "insert into tna (task,training,othertr,targetskill,currentskill,gap,trainingtype,monthapply,status,section,year,userid, department) values ('$taskes','$traininges','$otres','$targetskes','$currentskes','$gapes','$trtypees','$datetres','1','esgaware','2023','$userid', '$department')";
                mysqli_query($conn, $sql);
