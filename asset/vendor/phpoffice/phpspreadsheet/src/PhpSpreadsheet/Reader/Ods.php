...
//                                                                    ->setAuthor( $author )
    ...
    {
        $items = $settings->getElementsByTagNameNS($configNs, 'config-item');
        foreach ($items as $t) {
    ...
    {
        $items = $settings->getElementsByTagNameNS($configNs, 'config-item-map-named');
        foreach ($items as $t) {
            if (!($t instanceof DOMElement)) {
                continue;
            }
    ...
    private function setSelected(Spreadsheet $spreadsheet, string $wsname, string $setCol, string $setRow): void
    {
        if (is_numeric($setCol) && is_numeric($setRow)) {
            try {
                $spreadsheet->getSheetByName($wsname)->setSelectedCells([$setCol + 1, $setRow + 1]);
            } catch (Throwable $e) {
                // do nothing
            }
        }
