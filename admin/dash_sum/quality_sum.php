...
<html lang="en">
        ...
        <div class="container-fluid">
			...
			<div class="row">
                <div class="col-md-6">
					<div class="panel panel-default">
                        <div class="panel-heading">
                            <strong>Quality Management System Department</strong>
                        </div>
                        <div class="panel-body" align="center">
							<div class="row">
								<div class="col-md-12">
									<canvas id="qmsChart" style="width:100%;max-width:700px"></canvas>
								</div>
							</div>
						</div>
					</div>
				...
				<div class="col-md-6">
					<div class="panel panel-default">
                        <div class="panel-heading">
                            <strong>Quality Management (BB, TM 1 & TM 2) Department</strong>
                        </div>
                        <div class="panel-body" align="center">
							<div class="row">
								<div class="col-md-12">
									<canvas id="qmbbChart" style="width:100%;max-width:700px"></canvas>
								</div>
							</div>
						</div>
					</div>
			...
			<div class="row">
                <div class="col-md-6">
					<div class="panel panel-default">
                        <div class="panel-heading">
                            <strong>Quality Management (MLK & PKN) Department</strong>
                        </div>
                        <div class="panel-body" align="center">
							<div class="row">
								<div class="col-md-12">
									<canvas id="qmmlkChart" style="width:100%;max-width:700px"></canvas>
								</div>
							</div>
						</div>
					</div>
				...
				<div class="col-md-6">
					<div class="panel panel-default">
                        <div class="panel-heading">
                            <strong>Quality Management (SA 1 & SA 2) Department</strong>
                        </div>
                        <div class="panel-body" align="center">
							<div class="row">
								<div class="col-md-12">
									<canvas id="qmsaChart" style="width:100%;max-width:700px"></canvas>
								</div>
							</div>
						</div>
					</div>
    ...
    <script>
		...
		var canvasQms = document.getElementById("qmsChart");
		var qmsChart;

		function makeqmschart(mode,startdate,enddate) {
			$.ajax({
				url:"fetch_quality.php",
				method:"POST",
				data:{action:'fetch_qms',mode:mode,startdate:startdate,enddate:enddate},
				dataType:"JSON",
				success:function(data)
				{
                    var ctxQms = canvasQms.getContext('2d');
                    var category = [];
					var totalsend = [];
					var colorplant = [];
		...
		function destroyChartQms() {
            qmsChart.destroy();
        }

		makeqmschart(modeprev,fd,ld);

		var canvasQmbb = document.getElementById("qmbbChart");
		var qmbbChart;

		function makeqmbbchart(mode,startdate,enddate) {
			$.ajax({
				url:"fetch_quality.php",
				method:"POST",
				data:{action:'fetch_qmbb',mode:mode,startdate:startdate,enddate:enddate},
				dataType:"JSON",
				success:function(data)
				{
                    var ctxQmbb = canvasQmbb.getContext('2d');
                    var category = [];
					var totalsend = [];
					var colorplant = [];
		...
		function destroyChartQmbb() {
            qmbbChart.destroy();
        }

		makeqmbbchart(modeprev,fd,ld);

		var canvasQmmlk = document.getElementById("qmmlkChart");
		var qmmlkChart;

		function makeqmmlkchart(mode,startdate,enddate) {
			$.ajax({
				url:"fetch_quality.php",
				method:"POST",
				data:{action:'fetch_qmmlk',mode:mode,startdate:startdate,enddate:enddate},
				dataType:"JSON",
				success:function(data)
				{
                    var ctxQmmlk = canvasQmmlk.getContext('2d');
                    var category = [];
					var totalsend = [];
					var colorplant = [];
		...
		function destroyChartQmmlk() {
            qmmlkChart.destroy();
        }

		makeqmmlkchart(modeprev,fd,ld);

		var canvasQmsa = document.getElementById("qmsaChart");
		var qmsaChart;

		function makeqmsachart(mode,startdate,enddate) {
			$.ajax({
				url:"fetch_quality.php",
				method:"POST",
				data:{action:'fetch_qmsa',mode:mode,startdate:startdate,enddate:enddate},
				dataType:"JSON",
				success:function(data)
				{
                    var ctxQmsa = canvasQmsa.getContext('2d');
                    var category = [];
					var totalsend = [];
					var colorplant = [];
		...
		function destroyChartQmsa() {
            qmsaChart.destroy();
        }

		makeqmsachart(modeprev,fd,ld);
		...
		$('#filter_date').click(function(){
			...
			destroyChartQms();
			makeqmschart(mode,startdate,enddate);
			destroyChartQmbb();
			makeqmbbchart(mode,startdate,enddate);
			destroyChartQmmlk();
			makeqmmlkchart(mode,startdate,enddate);
			destroyChartQmsa();
			makeqmsachart(mode,startdate,enddate);
        });

        $('#clear_filter').click(function(){
			$('#datarecord').fadeIn().html('<label>Data summary : '+fd+' - '+ld+'</label');
			destroyChartQms();
			makeqmschart('manhour',fd,ld);
			destroyChartQmbb();
			makeqmbbchart('manhour',fd,ld);
			destroyChartQmmlk();
			makeqmmlkchart('manhour',fd,ld);
			destroyChartQmsa();
			makeqmsachart('manhour',fd,ld);
            $('#startdate').val('');
            $('#enddate').val('');
			$('#mode').val('manhour');
        });
