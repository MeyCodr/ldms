...
<html lang="en">
        ...
        <div class="container-fluid">
			...
			<div class="row">
                <div class="col-md-6">
					<div class="panel panel-default">
                        <div class="panel-heading">
                            <strong>Operation & Program Management Department</strong>
                        </div>
                        <div class="panel-body" align="center">
							<div class="row">
								<div class="col-md-12">
									<canvas id="onpmChart" style="width:100%;max-width:700px"></canvas>
								</div>
							</div>
						</div>
					</div>
				...
				<div class="col-md-6">
					<div class="panel panel-default">
                        <div class="panel-heading">
                            <strong>Manufacturing & SCM Department</strong>
                        </div>
                        <div class="panel-body" align="center">
							<div class="row">
								<div class="col-md-12">
									<canvas id="mnsChart" style="width:100%;max-width:700px"></canvas>
								</div>
							</div>
						</div>
					</div>
    ...
    <script>
		...
		var canvasOnpm = document.getElementById("onpmChart");
		var onpmChart;

		function makeonpmchart(mode,startdate,enddate) {
			$.ajax({
				url:"fetch_dhmsb.php",
				method:"POST",
				data:{action:'fetch_onpm',mode:mode,startdate:startdate,enddate:enddate},
				dataType:"JSON",
				success:function(data)
				{
                    var ctxOnpm = canvasOnpm.getContext('2d');
                    var category = [];
					var totalsend = [];
					var colorplant = [];
		...
		function destroyChartOnpm() {
            onpmChart.destroy();
        }

		makeonpmchart(modeprev,fd,ld);

		var canvasMns = document.getElementById("mnsChart");
		var mnsChart;

		function makemnschart(mode,startdate,enddate) {
			$.ajax({
				url:"fetch_dhmsb.php",
				method:"POST",
				data:{action:'fetch_mns',mode:mode,startdate:startdate,enddate:enddate},
				dataType:"JSON",
				success:function(data)
				{
                    var ctxMns = canvasMns.getContext('2d');
                    var category = [];
					var totalsend = [];
					var colorplant = [];
		...
		function destroyChartMns() {
            mnsChart.destroy();
        }

		makemnschart(modeprev,fd,ld);
		...
		$('#filter_date').click(function(){
			...
			destroyChartOnpm();
			makeonpmchart(mode,startdate,enddate);
			destroyChartMns();
			makemnschart(mode,startdate,enddate);
        });

        $('#clear_filter').click(function(){
			$('#datarecord').fadeIn().html('<label>Data summary : '+fd+' - '+ld+'</label');
			destroyChartOnpm();
			makeonpmchart('manhour',fd,ld);
			destroyChartMns();
			makemnschart('manhour',fd,ld);
            $('#startdate').val('');
            $('#enddate').val('');
			$('#mode').val('manhour');
        });
