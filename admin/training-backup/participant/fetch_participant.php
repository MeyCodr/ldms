<?php
    ...
    if($_POST["action"] == "load_session"){
		...
		$trainingid = $_POST['trainingid'];
        $output= array();
        $sql = "SELECT participation.id, participation.attendance, user.staffname,department FROM participation JOIN user ON participation.userid = user.`id` WHERE trainingid = '$trainingid';";
