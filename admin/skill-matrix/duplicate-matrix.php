...
if (isset($_SESSION['fullname']) && skillMatrixDuplicateUserCanUse()) {
    ...
    if ($sourceStaffId > 0) {
        if ($canUseSkillMatrix && (!isset($_SESSION['department']) || $_SESSION['department'] == '')) {
        ...
        if ($canUseSkillMatrix) {
            $staffSql .= " AND (dp.name = ? OR u.department = ?)";
        }

        $staffStmt = $conn->prepare($staffSql);
        if ($canUseSkillMatrix) {
            $sessionDepartment = isset($_SESSION['department']) ? $_SESSION['department'] : '';
            $staffStmt->bind_param("iss", $sourceStaffId, $sessionDepartment, $sessionDepartment);
        } else {
            $staffStmt->bind_param("i", $sourceStaffId);
        }
        $staffStmt->execute();
        $sourceStaff = $staffStmt->get_result()->fetch_assoc();

        if ($sourceStaff) {
            $department = $sourceStaff['department'];
