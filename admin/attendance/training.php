                        ...
                        <div class="panel-heading">
                            <div class="row">
                                <div class="col-md-6" style="margin-top: 10px;">
                                    <strong id="title">My Training Records</strong>
                                </div>
                                <div class="col-md-6" align="right">
                                    <button type="button" name="add_ojt" id="add_ojt" class="btn btn-success btn-md">Add
                                        Training <i class="far fa-arrow-alt-circle-right"></i> </button>
                                </div>
                            </div>
                        </div>
    ...
    <script>
        ...
        function fetch_data(action, userid, startdate, enddate) {
            var trainingdataTable = $('#traininglist').DataTable({
                ...
                "pageLength": 10,
                "info": true,
                "destroy": true, // important when reinitializing
                "ajax": {
                ...
                // Remove direct text-center for HTML columns to avoid style error
                "columnDefs": [
                    { className: 'text-center', targets: [0, 3, 4, 5, 7] } // only plain text columns
                ],

                "order": [[11, 'asc']], // sort by custom_sort hidden column

                "createdRow": function (row, data, dataIndex) {
                    // safely center HTML columns
                    $('td', row).eq(6).css('text-align', 'center'); // status
                    $('td', row).eq(8).css('text-align', 'center'); // pme
                    $('td', row).eq(9).css('text-align', 'center'); // btnedit
                },
                ...
                fnFooterCallback: function (nRow, aaData, iStart, iEnd, aiDisplay) {
                    ...
                    var totalhourall = api.column(7).data().reduce(function (a, b) { return intVal(a) + intVal(b); }, 0);
